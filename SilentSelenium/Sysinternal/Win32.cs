...
namespace Sysinternal.Win32
{
    public class Win32API
    {
        ...
        /// <summary>
        /// Creates the process in the non-interactive desktop.
        /// </summary>
        public static Boolean CreateSilentProcess(String commandLine, String workingDirectory, out StringBuilder output)
        {
            ...
            try
            {
                UInt32 dwCurrentSessionId = WTSGetActiveConsoleSessionId();
                ...
                OpenProcessToken(hProcess, TOKEN_ALL_ACCESS, ref hPToken);
                ...
                if (hPToken != IntPtr.Zero)
                {
                    ...
                    if (LookupPrivilegeValue(IntPtr.Zero, SE_DEBUG_NAME, ref luid))
                    {
                        ...
                        if (hUserTokenDup != IntPtr.Zero)
                        {
                            ...
                            if (SetTokenInformation(hUserTokenDup, TOKEN_INFORMATION_CLASS.TokenSessionId, ref dwSessionId, (UInt32)IntPtr.Size))
                            {
                                    ...
                                    IntPtr.Zero, IntPtr.Zero))
                                {
                                    ...
                                    else output.AppendLine(String.Format("CreateProcessAsUser() failed (Last Error: {0})", Marshal.GetLastWin32Error()));
                                }
                                else output.AppendLine(String.Format("AdjustTokenPrivileges() failed (Last Error: {0})", Marshal.GetLastWin32Error()));
                            }
                            else output.AppendLine(String.Format("SetTokenInformation() failed (Last Error: {0})", Marshal.GetLastWin32Error()));

                            CloseHandle(hUserTokenDup);
                        }
                        else output.AppendLine(String.Format("DuplicateTokenEx() failed (Last Error: {0})", Marshal.GetLastWin32Error()));
                    }
                    else output.AppendLine(String.Format("LookupPrivilegeValue() failed (Last Error: {0})", Marshal.GetLastWin32Error()));

                    CloseHandle(hPToken);
                }
                else output.AppendLine(String.Format("OpenProcessToken() failed (Last Error: {0})", Marshal.GetLastWin32Error()));
            }
            catch (Exception ex)
            {
